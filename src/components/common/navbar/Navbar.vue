    ...
    <header :class="{'header-filled': background}" class="header-big d-grid z-10">
        <nav class="flex flex-row justify-between items-center">
            ...
            <div class="nav-links">
                ...
                <NuxtLink :to="{name: 'paste'}" active-class="active" class="nav-link">
                    Paste
                </NuxtLink>
                ...
                <NuxtLink v-if="user?.role === 'admin'" :to="{name: 'admin'}" active-class="active" class="nav-link">
                    Admin
                </NuxtLink>
            </div>
            ...
            <div class="nav-button nav-side">
                <div
                    ...
                    id="user-menu-button"
                    aria-expanded="false"
                    class="nav-action account-link cursor-pointer relative"
                    data-dropdown-placement="bottom-end"
                    data-dropdown-toggle="user-dropdown"
                >
                    Account
                </div>
                <NuxtLink v-else class="nav-action" exact-active-class="lmWixQ" to="/login">
                    Login
                </NuxtLink>
                ...
                >
                    <div class="px-4 py-3">
                        <span :class="[!background ? 'text-gray-900': 'text-gray-100 dark:text-black']" class="block text-sm">
                    ...
                    <ul aria-labelledby="user-menu-button" class="py-2">
                        ...
                        <li>
                            <NavPopupItem :background="background" icon="right-from-bracket" label="Sign out" @click.prevent="logoutUser()" />
                        </li>
                    </ul>
                </div>
            </div>
