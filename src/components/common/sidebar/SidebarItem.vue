...
const isActive = (item: SidebarItem, checkForDefault = true) => {
    if (item.activeRequirements) {
        return true;
    }
    if (checkForDefault && isDefault(item)) {
        return true;
    }

    if ('name' in item.link && route.matched.filter(i => i.name === item.link.name).length === 0) {
        return false;
    }
    if ('query' in item.link) {
        for (const key in item.link.query) {
            if (route.query[key] !== item.link.query[key]) {
                return false;
            }
        }
    }
