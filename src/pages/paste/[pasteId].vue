...
const paste: Ref<Paste | undefined> = ref();
...
const fetchPaste = async () => {
    loading.value = true;
    try {
        paste.value = await PasteRepository.fetchPaste(pasteId);
    } catch (e) {
...
await fetchPaste();
useHead({
    title: paste.value?.title + ' - Paste | Devmart',
});

// computed
const formattedUpdatedAt = computed(() => {
    if (!paste.value?.updated_at) return null;
    return DateService.formatDateRelatively(paste.value?.updated_at, true);
});
